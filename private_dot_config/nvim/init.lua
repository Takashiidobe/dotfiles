...
local liblldb_path = is_mac and vim.fn.expand("~/.local/share/codelldb/lldb/lib/liblldb.dylib")
...
vim.api.nvim_create_user_command("CheckDeps", function()
	local deps = {
		...
		{ group = "Lean", name = "elan", cmd = "elan" },
		{ group = "Lean", name = "lake", cmd = "lake" },
		{ group = "Lean", name = "lean", cmd = "lean" },
	...
	for _, dep in ipairs(deps) do
		...
		local ok = dep.path and vim.uv.fs_stat(dep.cmd) ~= nil or vim.fn.executable(dep.cmd) == 1
	...
	vim.keymap.set(
		...
		"<Esc>",
		"<cmd>close<cr>",
		{ buffer = buf, silent = true, desc = "Close dependency check window" }
	)
end, {})
...
require("lazy").setup({
		...
		config = function()
			require("conform").setup({
				...
				formatters = {
					...
					-- },
					capnpfmt_dev = {
						command = "/home/takashi/yalspcc/target/generated-capnp/build/debug/capnpfmt",
						stdin = true,
					},
				},
	...
	-- LSP
	{
		...
		config = function()
			...
			-- Ruff for Python
			-- Configure optional settings for the server
			vim.lsp.config("ty", {
				settings = {
					ty = {
						-- Put your custom ty rule levels or settings here
					},
				},
			})
			...
			vim.api.nvim_create_autocmd("FileType", {
				...
				callback = function(args)
					vim.treesitter.start(args.buf, "lean")
				end,
			})
			...
			vim.api.nvim_create_autocmd("FileType", {
				pattern = "capnp",
				callback = function(args)
					vim.treesitter.start(args.buf, "capnp")
				end,
			})
			...
			-- testing out capnp lsp
			vim.lsp.config("capnprotols", {
				cmd = { "/home/takashi/yalspcc/target/generated-capnp/build/debug/capnprotols" },
				filetypes = { "capnp" },
				root_markers = { ".git" },
			})
			vim.lsp.enable("capnprotols")
			...
			end, { desc = "Diagnostics: previous item" })
			vim.keymap.set("n", "]d", function()
				vim.diagnostic.jump({ count = 1 })
			end, { desc = "Diagnostics: next item" })
			vim.keymap.set("n", "[e", function()
				vim.diagnostic.jump({ count = -1, severity = vim.diagnostic.severity.ERROR })
			end, { desc = "Diagnostics: previous error" })
			vim.keymap.set("n", "]e", function()
				vim.diagnostic.jump({ count = 1, severity = vim.diagnostic.severity.ERROR })
			...
			local function call_tree_ignored(uri)
				...
				for _, pat in ipairs(call_tree_ignore) do
					if uri:find(pat, 1, true) then
						return true
					end
					if ok and fname:find(pat, 1, true) then
						return true
					end
				end
				return false
			...
			local function call_tree(direction)
				...
				local function render()
					...
					for row, node in ipairs(visible) do
						...
						table.insert(
							lines,
							string.format("%s%s %s  %s:%d", node.line_prefix, glyph, qname, fname, lnum)
						)
						local r = row - 1
						...
						vim.schedule(function()
							...
							if calls then
								for _, call in ipairs(calls) do
									...
									if
										not call_tree_ignored(child_item.uri)
				...
				vim.keymap.set("n", "<CR>", function()
					...
					if node then
						jump({ uri = node.item.uri, range = node.item.selectionRange })
					end
				...
				vim.keymap.set("n", "q", function()
					vim.api.nvim_win_close(win, true)
				end, { buffer = buf, silent = true, desc = "Call tree: close window" })
							...
							if not items or #items == 0 then
								if attempt >= max_retries then
									...
									vim.api.nvim_buf_set_lines(
										...
										-1,
										false,
										{ "  No call hierarchy item at cursor" }
									)
									vim.bo[buf].modifiable = false
								else
			...
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(ev)
					...
					vim.keymap.set(
						...
						keymap_opts("LSP: code action", opts)
					)
					...
					vim.keymap.set("n", "<leader>ci", function()
						call_tree("incoming")
					end, keymap_opts("LSP: incoming call tree", opts))
					vim.keymap.set("n", "<leader>co", function()
						call_tree("outgoing")
					end, keymap_opts("LSP: outgoing call tree", opts))
					...
					vim.keymap.set("n", "<leader>ma", function()
						...
						if cargo_toml ~= "" then
							for line in io.lines(cargo_toml) do
								local name = line:match('^name%s*=%s*"([^"]+)"')
								if name then
									crate_name = name:gsub("-", "_")
								end
								if line:match("^%[lib%]") then
									has_lib = true
								end
							end
						end
						...
						local function run_asm(query, extra_args)
							...
							vim.keymap.set(
								...
								"q",
								"<cmd>close<cr>",
								{ buffer = buf, silent = true, desc = "Rust asm: close window" }
							)
									...
									vim.schedule(function()
										...
										if code ~= 0 then
											...
											for _, line in ipairs(output) do
												...
												if flag then
													table.insert(targets, vim.trim(flag))
												end
											end
											if #targets > 0 then
								...
								local function find_path(syms, prefix)
									for _, sym in ipairs(syms) do
										...
										if r and cursor_line >= r.start.line and cursor_line <= r["end"].line then
											local child_path = sym.children
												and find_path(sym.children, prefix .. "::" .. sym.name)
								...
								vim.schedule(function()
									...
									for _, line in ipairs(output) do
										...
										if desc then
											...
											local item, path, lnum =
												line:match("^%s+%w+%s+([^,]+), previously in file ([^:]+):(%d+)$")
											if item and path and lnum then
												local rel = path:match(
													"[/\\]target[/\\]semver%-checks[/\\][^/\\]+[/\\][0-9a-f]+[/\\](.+)$"
												)
									...
									if code == 0 or #qflist == 0 then
										vim.notify(
											code == 0 and "No semver violations"
												or "cargo semver-checks failed (no locations parsed)",
											vim.log.levels.INFO
										)
										return
									end
		...
		config = function()
			...
			treesitter.setup({
				local_parsers = {
					lean = {
						source = {
							-- The Lean repository includes its own queries/ directory.
							type = "self_contained",
	...
	-- testing
	{
		...
		config = function()
			...
			end, { desc = "Test: rerun last" })
			vim.keymap.set("n", "<leader>tq", function()
				require("neotest").run.stop()
			end, { desc = "Test: stop running test" })
			...
			end, { desc = "Test: next failed test" })
			vim.keymap.set("n", "[t", function()
				require("neotest").jump.prev({ status = "failed" })
			end, { desc = "Test: previous failed test" })
		end,
	},
	-- debugging
	{
					...
					program = function()
						...
						for line in output:gmatch("[^\n]+") do
							...
							if
								...
								and data.executable
								and data.profile
								and data.profile.test
								and data.target
								and data.target.kind
								and vim.tbl_contains(data.target.kind, "lib")
							then
	...
	},
	{
		...
		config = function()
			...
			vim.keymap.set(
				...
				{ desc = "Debug: clear all persistent breakpoints" }
			)
		end,
	},
	{
	...
	},
	{
		...
		config = function()
			...
			vim.keymap.set(
				...
				keymap_opts("Crates: show dependencies", opts)
			)
			...
			vim.keymap.set(
				...
				keymap_opts("Crates: upgrade selected crates", opts)
			)
			vim.keymap.set(
				"n",
				"<leader>cA",
				crates.upgrade_all_crates,
				keymap_opts("Crates: upgrade all crates", opts)
			)
			...
			vim.keymap.set(
				...
				keymap_opts("Crates: open documentation", opts)
			)
	...
	},
	{
		...
		keys = {
			...
			},
			{
				...
				desc = "GitHub Issues (assigned to me)",
			},
			{
				"<leader>gin",
				function()
					Snacks.picker.gh_issue({ mention = "@me" })
				end,
				desc = "GitHub Issues (mentioning me)",
			},
			{
				"<leader>gil",
				function()
					vim.ui.input({ prompt = "Label: " }, function(label)
						if label and label ~= "" then
							Snacks.picker.gh_issue({ label = label })
						end
					end)
				end,
			...
			},
			{
				...
				desc = "GitHub PRs (assigned to me)",
			},
			{
				"<leader>gpn",
				function()
					Snacks.picker.gh_pr({ involves = "@me" })
				end,
				desc = "GitHub PRs (involving me)",
			},
			{
				"<leader>gpl",
				function()
					vim.ui.input({ prompt = "Label: " }, function(label)
						if label and label ~= "" then
							Snacks.picker.gh_pr({ label = label })
						end
					end)
				end,
